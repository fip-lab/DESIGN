...
import numpy as np
import torch
from sklearn.metrics import recall_score, precision_score, average_precision_score, classification_report, f1_score,confusion_matrix
...
def custom_recall(x, y):
    cm = confusion_matrix(x, y)
    recalls = []
    for i in range(cm.shape[0]):
        if cm[i].sum() == 0:  # 如果某个类没有实际实例，其召回率为0
            recalls.append(0)
        else:
            recalls.append(cm[i][i] / cm[i].sum())  # 召回率计算公式：TP / (TP + FN)
    
    # 如果数据只包含一个类，手动添加另一个类的召回率
    if len(recalls) == 1:
        if 0 in x:
            recalls.append(0)  # 如果只有0类，那么1类的召回率为0
        else:
            recalls.insert(0, 0)  # 如果只有1类，那么0类的召回率为0
    return recalls
...
def get_cls_report(y_true, y_pred):
    ...
    if len(precision) == 1:
        if 0 in y_true:
            precision = list(precision) + [0]
        else:
            precision = [0] + list(precision)
...
def main():
    ...
    parser.add_argument("--selection_knowledge_type", type=str, choices=["faq","review","both"])
