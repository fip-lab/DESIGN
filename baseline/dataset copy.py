...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(self, args, tokenizer, split_type, labels=True, labels_file=None):
        ...
        self.speaker1, self.speaker2, self.knowledge_sep, self.knowledge_tag, self.faq_tag, self.review_tag, self.user_query, self.aspect_item = self.tokenizer.convert_tokens_to_ids(
        ...
        self.both_knowledge_prompt = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize("Whether the faq or review matches the user query and its aspect items. 1 is a match, 0 is not a match"))
    ...
    def _prepare_conversations(self):
        ...
        tokenized_dialogs = []
        ##JT
        for i, (log, label, log_aspect) in enumerate(tqdm(self.dataset_walker, disable=False, desc='tokenizing...')):
            dialog = {}
            dialog["id"] = i
            dialog["log"] = log
    ...
    def _create_examples(self):
        ...
        for dialog in tqdm(self.dialogs, disable=False, desc='creating examples'):
            ...
            dialog_id = dialog["id"]
            label = dialog["label"]
            query_aspect_term = dialog["log_aspect"]
                ...
                for knowledge in label_knowledge:
                    ...
                    if prefix not in knowledge_prefix_visited:
                        ...
                        if knowledge["domain"] == "restaurant":
                            is_specific_dish_or_drink, item_list = self._specific_dish_or_drink(user_last_utterance, prefix)
                            if is_specific_dish_or_drink == True:
                            ##_knowledge_candidates只加入匹配到的具体drink或者dish的顾客的review
                                knowledge_docs = knowledge_base["restaurant"][str(knowledge["entity_id"])]["reviews"]
                                candidates = []
                                for i in range(len(knowledge_docs)):
                                    for item in item_list:
                                        if item in knowledge_docs[str(i)]["drinks"] or item in knowledge_docs[str(i)]["dishes"]:
                                            #(f"{knowledge['domain']}__{knowledge['entity_id']}__{i}")
                                            knowledge_sentence_doc = knowledge_docs[str(i)]["sentences"]
                                            for key in knowledge_sentence_doc.keys():
                                                knowledge_key = f"{knowledge['domain']}__{knowledge['entity_id']}__{i}-{key}"
                                                candidates.append(knowledge_key)
                                                _knowledge_candidates = candidates
                                            
                                            _knowledge_candidates.append(
                                                cand
                                                for cand in self.snippets.keys()
                                                if ('-' in cand) and (cand.split('-')[0] == f"{knowledge['domain']}__{knowledge['entity_id']}__{i}")
                                            )
            ...
            self.examples.append({
                ...
                ##JT
                "query_ascept_item": query_aspect_term,  ##aspect term  (string)
...
class KnowledgeSelectionDataset(BaseDataset):
    ...
    def build_input_from_segments(self, knowledge, history, aspect_item):
        ...
        aspect_sentence_tokenized = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(aspect_sentence))

        if self.args.selection_knowledge_type == 'review':
            sequence0 = [self.cls] + [self.user_query] + history[-1] + [self.sep] + [self.aspect_item] + aspect_sentence_tokenized + [self.sep] + [self.review_tag] + knowledge + [self.sep]
            sequence1 = self.review_prompt + [self.sep]
        elif self.args.selection_knowledge_type == 'faq':
            sequence0 = [self.cls] + [self.user_query] + history[-1] + [self.sep] + [self.aspect_item] + aspect_sentence_tokenized + [self.sep] + [self.faq_tag] + knowledge + [self.sep]
