...
import os
os.environ["CUDA_VISIBLE_DEVICES"] = "0"  # 如果你的两张卡的编号是0和1
...
def process_data(data, alignment_type):
    inputs = []
    labels = []
    for item in tqdm(data,desc="process data"):
        inputs.append(create_few_shot(item['user_query'], item['aspect_item_of_query'], item['knowledge_sentence'],item["triple"], alignment_type))
...
train_data = load_data("/home/zhuangjt/zhuangjt_disk3/SK-TOD/InstructKS/Dynamic_InstructKS_Dataset_triple_based/train_dataset_2.json")
val_data = load_data("/home/zhuangjt/zhuangjt_disk3/SK-TOD/InstructKS/Dynamic_InstructKS_Dataset_triple_based/val_dataset.json")

##改进:对训练集进行采样，选取所有labels为yes的数据，然后随机采样与正样本数量相同的负样本(labels为no)


train_texts, train_labels = process_data(train_data, alignment_type)
val_texts, val_labels = process_data(val_data, alignment_type)
...
training_args = Seq2SeqTrainingArguments(
    ...
    do_train=True,
    do_eval=True,
    output_dir = f"./results/{alignment_type}-{negative_sample_size}neg{positive_sample_size}pos-reviewer",
