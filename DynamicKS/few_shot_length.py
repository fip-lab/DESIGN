...
output:""".format(user_query,aspect_item_of_query,knowledge_sentence)
    ...
    #计算input_text的token数量
    token_num = len(tokenizer(input_text)['input_ids'])

    return input_text,token_num
...
def process_data(data):
    ...
    for item in tqdm(data,desc="process data"):
        ...
        inputs.append(create_few_shot(item['user_query'], item['aspect_item_of_query'], item['knowledge_sentence'])[0])
        token_nums.append(create_few_shot(item['user_query'], item['aspect_item_of_query'], item['knowledge_sentence'])[1])
        labels.append(item['labels'])
    return inputs, labels, token_nums
...
train_texts, train_labels, train_token_num = process_data(train_data)
val_texts, val_labels,val_token_num = process_data(val_data)

#分别计算train和val的token数量的平均值和最大值
train_token_num_mean = np.mean(train_token_num)
train_token_num_max = np.max(train_token_num)
val_token_num_mean = np.mean(val_token_num)
val_token_num_max = np.max(val_token_num)

print("train_token_num_mean:",train_token_num_mean)
print("train_token_num_max:",train_token_num_max)
print("val_token_num_mean:",val_token_num_mean)
print("val_token_num_max:",val_token_num_max)
