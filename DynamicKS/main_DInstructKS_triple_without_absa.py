...
##few-shot构建函数
def create_few_shot(user_query, aspect_item_of_query, knowledge_sentence, triple):
    ##这个函数主要是用于构建few-shot
    ##输入是用户的query
...
output:""".format(user_query,knowledge_sentence)
    ...
    if len(positive_knowledges_text) >=2:
        positive_knowledges_text = random.sample(positive_knowledges_text,2)
...
def process_data(data):
    ...
    for item in tqdm(data,desc="process data"):
        inputs.append(create_few_shot(item['user_query'], item['aspect_item_of_query'], item['knowledge_sentence'],item["triple"]))
...
training_args = Seq2SeqTrainingArguments(
    ...
    do_train=True,
    do_eval=True,
    output_dir='./results/DInstructKS-flan-triple-without-absa',
