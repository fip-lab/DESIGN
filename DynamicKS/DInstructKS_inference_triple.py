...
parser.add_argument('--alignment_type', type=str, default='triple', help='Alignment type')
...
print("device:",device,"dataset_split:",dataset_split,"alignment_type:",alignment_type,"positive_sample_size:",positive_sample_size,"negative_sample_size:",negative_sample_size)

# 加载 tokenizer 和模型
tokenizer = AutoTokenizer.from_pretrained(f'/home/zhuangjt/zhuangjt_disk3/SK-TOD/results/{model_name}')  # 指向保存 tokenizer 的目录
model = AutoModelForSeq2SeqLM.from_pretrained(f'/home/zhuangjt/zhuangjt_disk3/SK-TOD/results/{model_name}')  # 指向保存模型的目录
model.to(device)
...
triples = load_data(f"/home/zhuangjt/zhuangjt_disk3/SK-TOD/data/{dataset_split}/triple.json")
...
##加载资源向量库
if alignment_type == "triple":
    resourse_vector = np.load('/home/zhuangjt/zhuangjt_disk3/SK-TOD/InstructKS/Dynamic_InstructKS_Dataset_triple_based/resourse/resource_triple_vector_base.npy')
elif alignment_type == "splice":
    resourse_vector = np.load('/home/zhuangjt/zhuangjt_disk3/SK-TOD/InstructKS/Dynamic_InstructKS_Dataset_triple_based/resourse/resource_splice.npy')
elif alignment_type == "summation":
    resourse_vector = np.load('/home/zhuangjt/zhuangjt_disk3/SK-TOD/InstructKS/Dynamic_InstructKS_Dataset_triple_based/resourse/resource_summation.npy')
elif alignment_type == "average":
    resourse_vector = np.load('/home/zhuangjt/zhuangjt_disk3/SK-TOD/InstructKS/Dynamic_InstructKS_Dataset_triple_based/resourse/resource_average.npy')
else:
    print("Please input the correct alignment type!")

dimension = resourse_vector.shape[1]
index = faiss.IndexFlatL2(dimension)
index.add(resourse_vector)

resourse_labels = load_data('/home/zhuangjt/zhuangjt_disk3/SK-TOD/InstructKS/Dynamic_InstructKS_Dataset_triple_based/resourse/labels.json')
resourse_logs = load_data('/home/zhuangjt/zhuangjt_disk3/SK-TOD/InstructKS/Dynamic_InstructKS_Dataset_triple_based/resourse/logs.json')
resourse_logs_aspect = load_data('/home/zhuangjt/zhuangjt_disk3/SK-TOD/InstructKS/Dynamic_InstructKS_Dataset_triple_based/resourse/logs_aspect.json')
resourse_triples = load_data('/home/zhuangjt/zhuangjt_disk3/SK-TOD/InstructKS/Dynamic_InstructKS_Dataset_triple_based/resourse/triple.json')

##先对resourse进行处理，构建三个列表，把所有label["target"] == false的排除掉
resourse_labels_new = []
resourse_logs_new = []
resourse_logs_aspect_new = []
resourse_triples_new = []
for label,log,log_aspect, triple in zip(resourse_labels,resourse_logs,resourse_logs_aspect,resourse_triples):
    if label["target"] == True:
        ...
        resourse_triples_new.append(triple)
...
##few-shot构建函数
def create_few_shot(user_query, aspect_item_of_query, knowledge_sentence, triple, alignment_type):
    ##这个函数主要是用于构建few-shot
    ##输入是用户的query
...
output:""".format(user_query,aspect_item_of_query,knowledge_sentence)
    ...
    ##计算query的向量
    if alignment_type == "triple":
        query_embedding = similarity_model.encode([triple])
    elif alignment_type == "splice":
        sentence_embedding = similarity_model.encode([user_query])
        triple_embedding = similarity_model.encode([triple])
        sentence_embedding = np.array(sentence_embedding)
        triple_embedding = np.array(triple_embedding)
        query_embedding = np.concatenate((sentence_embedding,triple_embedding),axis=1)
    elif alignment_type == "summation":
        sentence_embedding = similarity_model.encode([user_query])
        triple_embedding = similarity_model.encode([triple])
        sentence_embedding = np.array(sentence_embedding)
        triple_embedding = np.array(triple_embedding)
        query_embedding = sentence_embedding + triple_embedding
    elif alignment_type == "average":
        sentence_embedding = similarity_model.encode([user_query])
        triple_embedding = similarity_model.encode([triple])
        sentence_embedding = np.array(sentence_embedding)
        triple_embedding = np.array(triple_embedding)
        query_embedding = (sentence_embedding + triple_embedding)/2
...
def instructks(knowledge_sent, user_query, aspect_item_of_query, triple, alignment_type):
    input_text = create_few_shot(user_query, aspect_item_of_query, knowledge_sent, triple, alignment_type)
...
save_path = f"/home/zhuangjt/zhuangjt_disk3/SK-TOD/pred/{dataset_split}/DEALinKS_{negative_sample_size}neg{positive_sample_size}pos.json"

list = []
##加载emm_knowledge,logs,logs_aspect
for idx,(em, log, log_aspect, triple) in enumerate(tqdm(zip(em_knowledge, logs, logs_aspect, triples))):
    dict = {}
    if em["target"] == False:
        dict["target"] = False
        list.append(dict)
        ...
        for knowledge_info in knowledge_infs:  
            ...
            for review_doc_id in knowledge_reader.get_review_doc_ids(domain, entity_id):
                ...
                for review_sent_id, review_sent in review_doc['sentences'].items():
                    ...
                    response = instructks(knowledge_sent, user_query, aspect_item_of_query, triple,alignment_type)
                    if response == True:
                        dict_know = {}
