...
parser.add_argument('--entailment_ratio', type=float, default=0.8, help='Entailment ratio')
parser.add_argument('--interest_ratio', type=float, default=0.2, help='Interest ratio')
...
Answer:""".format(knowledge_text,query)
            ...
            with torch.no_grad():
                outputs = model.generate(inputs.input_ids, num_beams=5, num_return_sequences=5, max_length=100, early_stopping=True)
            ...
            #按照0.8*entailment_score+0.2*interest_score排序
            scores = [args.entailment_ratio * entailment_score + args.interest_ratio * interest_score for entailment_score, interest_score in zip(entailment_scores, interest_scores)]
            
            best_response_index = scores.index(max(scores))
            
            
            label["response"] = responses[best_response_index]
