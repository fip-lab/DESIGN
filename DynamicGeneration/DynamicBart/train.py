...
parser.add_argument('--output_model_path', type=str, help='Output path')
...
tokenizer = BartTokenizer.from_pretrained(f'/disk4/zhuangjt/zhuangjt_disk3/SK-TOD/PLM/{args.model_name}')
model = BartForConditionalGeneration.from_pretrained(f'/disk4/zhuangjt/zhuangjt_disk3/SK-TOD/PLM/{args.model_name}')
similarity_model = SentenceTransformer('/disk4/zhuangjt/zhuangjt_disk3/SK-TOD/PLM/all-MiniLM-L6-v2')
##加载资源向量库

resourse_vector = np.load('/disk4/zhuangjt/zhuangjt_disk3/SK-TOD/DynamicGeneration/Dynamic_Dataset/resourse/resource_vector_base.npy')
dimension = resourse_vector.shape[1]
index = faiss.IndexFlatL2(dimension)
index.add(resourse_vector)

resourse_labels = load_data('/disk4/zhuangjt/zhuangjt_disk3/SK-TOD/DynamicGeneration/Dynamic_Dataset/resourse/labels.json')
resourse_logs = load_data('/disk4/zhuangjt/zhuangjt_disk3/SK-TOD/DynamicGeneration/Dynamic_Dataset/resourse/logs.json')
resourse_logs_aspect = load_data('/disk4/zhuangjt/zhuangjt_disk3/SK-TOD/DynamicGeneration/Dynamic_Dataset/resourse/logs_aspect.json')
resourse_labels_new = []
resourse_logs_new = []
...
#路径由data_path传入，比如labels_train的路径应该是args.data_path + "/train/labels.json"
labels_train = load_data(args.data_path + "/train/labels.json")
logs_train = load_data(args.data_path + "/train/logs.json")
labels_val = load_data(args.data_path + "/val/labels.json")
logs_val = load_data(args.data_path + "/val/logs.json")
...
if args.withabsa and args.select_algorithm == 'RANDOM':
    output_path = f"./results/{args.model_name}-{args.top_k_shot}-shot-Random-earlystop-absa-7e-5"
elif args.withabsa and args.select_algorithm == 'SELECT':
    output_path = f"./results/{args.model_name}-{args.top_k_shot}-shot-earlystop-absa-7e-5"
elif not args.withabsa and args.select_algorithm == 'RANDOM':
    output_path = f"./results/{args.model_name}-{args.top_k_shot}-shot-withoutabsa-Random-earlystop-7e-5"
elif not args.withabsa and args.select_algorithm == 'SELECT':
    output_path = f"./results/{args.model_name}-{args.top_k_shot}-shot-withoutabsa-earlystop-7e-5"
"""

training_args = Seq2SeqTrainingArguments(
    output_dir= args.output_model_path,                 # 输出目录
    ...
    learning_rate=7e-5,                     # 学习率    learning_rate=3e-5,                     # 学习率
