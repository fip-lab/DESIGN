...
new_tokens = ["<user>", "<system>","<knowledge_sep>", "<review>", "<faq>", "<entity>"]
tokenizer.add_tokens(new_tokens)

user_token = new_tokens[0]
system_token = new_tokens[1]
knowledge_sep_token = new_tokens[2]
review_token = new_tokens[3]
faq_token = new_tokens[4]
entity_token = new_tokens[5]
...
def process_data(examples,instruct):
    ...
    inputs = [ex["knowledge"] + ex["dialog"] for ex in examples]
    #print(inputs[:20])
...
def main():
    ...
    padding = "max_length"
    train_encodings = tokenizer(train_texts, truncation=True, padding=padding, max_length=512)
    val_encodings = tokenizer(val_texts, truncation=True, padding=padding, max_length=512)
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        do_train=True,
        do_eval=True,
        output_dir=f"./checkpoint3/{args.knowledge_organization_mode}_{args.with_absa}_1205",
