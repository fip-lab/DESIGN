...
class KnowledgeReader(object):
    ...
    def get_review_absa(self, domain, entity_id, doc_id, sent_id):
        if domain not in self.get_domain_list():
            raise ValueError("invalid domain name: %s" % domain)
        ...
        result = self.knowledge[domain][str(entity_id)]['reviews'][str(doc_id)]['aspect_sentiment'][int(sent_id)]

        return result
    
    def get_faq_absa(self, domain, entity_id, doc_id):
        if domain not in self.get_domain_list():
            raise ValueError("invalid domain name: %s" % domain)
        ...
        result = self.knowledge[domain][str(entity_id)]['faqs'][str(doc_id)]['aspect_sentiment']
        return result