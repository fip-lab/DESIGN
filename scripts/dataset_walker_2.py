...
class DatasetWalker(object):
    def __init__(self, dataset, dataroot, labels=False, labels_file=None, incl_knowledge=False):
        ...
        ##JT
        logs_aspect_file = os.path.join(path, dataset, 'logs_aspect.json')
        with open(logs_aspect_file, 'r') as f:
            self.logs_aspect = json.load(f)
        triple_file = os.path.join(path, dataset, 'triple.json')
        with open(triple_file, 'r') as f:
            self.triple = json.load(f)
        
        logs_absa_file = os.path.join(path, dataset, 'logs_absa.json')
        with open(logs_absa_file, 'r') as f:
            self.logs_absa = json.load(f)
    ...
    def __iter__(self):
        if self.labels is not None:
            #JT
            for log, label, logs_absa, triple in zip(self.logs, self.labels, self.logs_absa, self.triple):
                ...
                yield(log, label, logs_absa, triple)
        else:
            for log in self.logs:
                yield(log, None,logs_absa, triple)
