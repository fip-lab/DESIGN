...
class DatasetWalker(object):
    ...
    def __iter__(self):
        if self.labels is not None:
            #JT
            #for log, label, log_aspect, triple in zip(self.logs, self.labels, self.logs_aspect, self.triple):
            for log, label, log_aspect in zip(self.logs, self.labels, self.logs_aspect):
                ...
                #yield(log, label, log_aspect, triple)
                yield(log, label, log_aspect)
        else:
            for log in self.logs:
                #yield(log, None,log_aspect, triple)
                yield(log, label, log_aspect)
