...
def load_models(device,name="/home/zhuangjt/zhuangjt_disk3/SK-TOD/PLM/DialoGPT-large"):
  tokenizer = AutoTokenizer.from_pretrained(name)
  model = AutoModelWithLMHead.from_pretrained(name)
  model.to(device)
  return model, tokenizer

def score(text, tokenizer, model, device):
...
def evaluate(conversation, model, tokenizer, device):
  scores = {}
  turn_level_utts = {
    "interesting": {
  ...
  for metric,utts in turn_level_utts.items():
    ...
    high_score = 0
    for m in pos:
      hs = score(conversation + " <|endoftext|> " + m, tokenizer, model, device) 
      high_score += hs 
    ...
    low_score = 0
    for m in neg:
      ls = score(conversation + " <|endoftext|> " + m, tokenizer, model, device) 
      low_score += ls 
